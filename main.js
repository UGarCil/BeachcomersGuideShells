...
const background_container = document.querySelector('#parent');
...
renderer.setSize(background_container.innerWidth, background_container.innerHeight); // Set the size of the renderer to match the window dimensions
...
function retrieveListOfCameras(scene) {
  ...
  // Get information on the initial position and rotation of the camera, and store the information as a new object
  ...
  scene.traverse(function (object) {
    ...
    if (object.name.includes("shell")) {
      retopoFlowObject.push(object);
      console.log(retopoFlowObject)
      console.log('RetopoFlow001 found:', retopoFlowObject);
    }
  });
...
function updateCameraAspect() {
  const width = background_container.clientWidth; // Get the width of the window
  const height = background_container.clientHeight; // Get the height of the window
...
function animate() {
  ...
  if (mixers) { // Check if the mixers array is defined (i.e., if animations are loaded)
    mixers.forEach(mixer => mixer.update(deltaTime * 0.05)); // Update all animations using their mixers
  }

  retopoFlowObject.forEach(retopo => {
    retopo.rotation.y += 0.008;
    retopo.rotation.x += 0.002;
    
  })
  // retopoFlowObject.rotation.z -= 0.004;
...
function moveCamera() {
  ...
  // retopoFlowObject.position.x = mapParameter(t, 0, 1*2, 2*2,3*2,4*2);
